...
from src.data_structure_operations import is_empty, split


def get_number_of_arguments_required(expectation_name: str) -> int:
    """
    Returns the number of parameters required by a specific
    expectation.
    :param expectation_name: String with the name of the expectation.
    :return: Int with the number of parameters that the expectation
    requires.
    """
    return defaults.EXPECTATION_N_PARAMS[expectation_name]
...
def check_number_of_params_is_correct(expectation_name: str, parameters: list) -> bool:
    """
    Checks if the number of parameters for an expectation is correct,
    as the name suggests.
    :param expectation_name: Str that contains the expectation name.
    :param parameters: List that contains all parameters that have to
    be checked.
    :return: Bool that tells the app if the number of parameters for
    the current expectation is correct or not.
    """
    if requires_parameters(expectation_name):
        ...
        if expectation_name == defaults.LENGTH_EXPECTATION_ID:
            if len(parameters) != 1:
                return False
...
def check_order_of_parameters_is_correct(parameters: list) -> bool:
    """
    Returns if at an expectation with parameters of type Min,Max the
    second value is equal or greater than the first one.
    :param parameters: List with extracted parameters.
    :return: Bool that tells the app if this requirement is
    accomplished.
    """
...
def check_params_type_is_correct(parameters: list) -> bool:
    """
    Checks if at an expectation with parameters of type Min,Max both
    values are integers
...
def extract_parameters(parameters: str) -> list or None:
    ...
    if parameters:
        ...
        parameters_list = split(parameters, ",")
